import { getLocalStorage, userDetails } from '../../common/common';
...
export const useOpenPixel = () => {
  // Open Pixel v1.3.1 | Published By Dockwa | Created By Stuart Yamartino | MIT License
  return function openPixelInitialize(window, document, pixelFunc, pixelFuncName, pixelEndpoint) {
    'use strict';
            ...
            return _objectSpread(
              {
                user: function userDetails() {
                  return getLocalStorage('user', undefined)?.user_master_id;
                },
                id: function id() {
                  return Config.id;
                },
    ...
    Cookie.exists('uid')
      ...
      : Cookie.set('uid', Helper.guid(), 2 * 365 * 24 * 60); // save any utms through as session cookies
    ...
    pixelFunc.process = function (method, value, optional) {
      if (method === 'init') {
        ...
        if (value !== 'pageload' && value !== 'pageclose') {
    ...
    var isSafari =
      (typeof safari === 'undefined' ? 'undefined' : _typeof(safari)) === 'object' &&
    ...
    for (var i = 0, l = dataAttributes.length; i < l; i++) {
      dataAttributes[i].addEventListener('click', function () {
        // alert('scrolled');
      });
    }

    dataAttributes.forEach((elem) => console.log(elem));
  ...
  // openPixelInitialize(window, document, window["opix"], "opix", "https://apigl.clirnet.com/openapi/trackingToken", 1);
};
